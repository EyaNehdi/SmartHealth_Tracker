...
<div class="container-fluid">
    ...
    <div class="enhanced-breadcrumb">
        ...
        <div class="d-flex justify-content-between align-items-center">
            <h1 class="h3 mb-0">Meal Plans Management</h1>
            ...
            <div class="search-container">
                ...
                <div class="search-section">
                    <div class="form-group-enhanced">
                        <label for="search" class="form-label-enhanced">
                            <i class="fas fa-search me-2"></i>Search Meal Plans
                        </label>
                        <div class="input-wrapper">
                            <input type="text"
                                ...
                                placeholder="Type to search meal plans..."
                                autocomplete="off">
                ...
                <div class="filters-section meals-filters">
                    <h6 class="filters-title">
                        <i class="fas fa-filter me-2"></i>Meal Plan Filters
                    </h6>
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            <div class="form-group-enhanced">
                                <label class="form-label-enhanced">Days Range</label>
                                    ...
                                    <div class="filter-options">
                                        <label class="filter-toggle">
                                            <input type="checkbox" name="statuses[]" value="active"
                                                {{ in_array('active', request('statuses', [])) ? 'checked' : '' }}>
                                            <span class="toggle-label">Active</span>
                                        </label>
                                        <label class="filter-toggle">
                                            <input type="checkbox" name="statuses[]" value="inactive"
                                                {{ in_array('inactive', request('statuses', [])) ? 'checked' : '' }}>
    ...
    <!-- Meal Plans Grid -->
    @include('backoffice.meal-plans.partials.meal-plans-grid')
</div>
@endsection

@push('backoffice-scripts')
<style>
    /* Professional Meal Plan Card Styling */
    .meal-plan-card {
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function updateRangeDisplay(nutrient, min, max) {
            const display = document.getElementById(`${nutrient}_range_display`);
            const unit = nutrient === 'meals_count' ? 'meals' : '';
            if (display) {
                display.textContent = `${min.toFixed(0)} - ${max.toFixed(0)} ${unit}`;
            }
        }
        ...
        function applyFilters(async = false) {
            ...
            ['duration_types', 'statuses', 'min_days', 'max_days'].forEach(param => {
                // For array parameters, remove all instances
                if (param === 'duration_types') {
                    url.searchParams.delete('duration_types[]');
            ...
            // Add duration types - only add checked ones
            const durationTypeCheckboxes = document.querySelectorAll('input[name="duration_types[]"]:checked');
            durationTypeCheckboxes.forEach(checkbox => {
                url.searchParams.append('duration_types[]', checkbox.value);
            });

            // Add statuses - only add checked ones
            const statusCheckboxes = document.querySelectorAll('input[name="statuses[]"]:checked');
            statusCheckboxes.forEach(checkbox => {
                url.searchParams.append('statuses[]', checkbox.value);
            });

            // Add days range values
            ...
            if (minDaysInput && minDaysInput.value && minDaysInput.value.trim() !== '') {
                url.searchParams.set('min_days', minDaysInput.value);
            }
            
            if (maxDaysInput && maxDaysInput.value && maxDaysInput.value.trim() !== '') {
                url.searchParams.set('max_days', maxDaysInput.value);
            }

            // Add range values (for sliders if any)
            sliders.forEach(nutrient => {
                const minInput = document.getElementById(`${nutrient}_min`);
                const maxInput = document.getElementById(`${nutrient}_max`);
        ...
        function clearFilters(async = false) {
            ...
            ['search', 'duration_types', 'statuses', 'min_days', 'max_days'].forEach(param => {
                if (param === 'duration_types') {
                    url.searchParams.delete('duration_types[]');
                } else if (param === 'statuses') {
                    url.searchParams.delete('statuses[]');
            ...
            // Reset checkboxes
            document.querySelectorAll('input[name="duration_types[]"], input[name="statuses[]"]').forEach(checkbox => {
                checkbox.checked = false;
            });
            
            // Reset days range inputs
            const minDaysInput = document.querySelector('input[name="min_days"]');
            const maxDaysInput = document.querySelector('input[name="max_days"]');
