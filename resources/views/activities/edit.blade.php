       ...
       <form method="POST" action="{{ route('activities.update', $activity->id) }}">
            ...
            <div class="form-grp">
                ...
                <select name="category_id" id="category_id" class="form-control @error('category_id') is-invalid @enderror">
                    ...
                    @foreach ($categories as $category)
                        <option value="{{ $category->id }}" {{ old('category_id', $activity->category_id) == $category->id ? 'selected' : '' }}>{{ $category->nom }}</option>
                    @endforeach
                </select>
                @error('category_id')
                    <div class="invalid-feedback">{{ $message }}</div>
            ...
            <div class="form-grp">
                ...
                <div class="form-check">
                    <input type="checkbox" name="completed" id="completed" value="1" class="form-check-input @error('completed') is-invalid @enderror" {{ old('completed', $activity->completed) ? 'checked' : '' }}>
                    @error('completed')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
