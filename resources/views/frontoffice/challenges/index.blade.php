    ...
    <div class="blog__post-content-four">
        <div class="blog__post-meta blog__post-meta-three">
            <ul class="list-wrap">
                ...
                <li class="flex items-center gap-3 bg-gray-100 rounded-xl px-4 py-2 w-max shadow-lg">
                    ...
                    <div class="flex flex-col">
                        <span class="text-xl font-bold">{{ $challenge->participations_count }}</span>
                        <span class="text-sm text-gray-700">Participants</span>
                    </div>
                </li>
            </ul>
        </div>
        <h2 class="title"><a href="blog-details.html">{{ $challenge->titre }}</a></h2>
        <p>{{ $challenge->description }}</p>

        {{-- Edit & Delete buttons only for creator --}}
        @if (auth()->id() === $challenge->created_by)
            <a href="{{ route('challenges.edit', $challenge->id) }}" class="tg-btn tg-btn-three">Edit</a>
        ...
        {{-- Show "See Group Chat" for creator or participants, "Participer" for others --}}
        @if (auth()->check() && $challenge->isParticipatedBy(auth()->id()))
            <a href="{{ route('groups.index', $challenge->id) }}" class="tg-btn tg-btn-three">See Group Chat</a>
        @else
            <a href="#" class="tg-btn tg-btn-three" data-bs-toggle="modal"
                data-bs-target="#participationModal{{ $challenge->id }}">Participer</a>
        @endif
                ...
                <div class="modal-content">
                    <form action="{{ route('participations.store') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="mb-3">
                                <label for="comment{{ $challenge->id }}" class="form-label">Commentaire</label>
                                <textarea name="comment" id="comment{{ $challenge->id }}" class="form-control"></textarea>
                            </div>
                            <div class="mb-3">
                                <label for="image{{ $challenge->id }}" class="form-label">Image</label>
                                <input type="file" name="image" id="image{{ $challenge->id }}" class="form-control"
                                    accept="image/*">
                            </div>
                        </div>
